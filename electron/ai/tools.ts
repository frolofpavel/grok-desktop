...
Возвращает количество применённых блоков. Требует подтверждения пользователя как обычный write — diff отображается в том же модальном окне.`,
  ...
  },
  {
    ...
    description: 'Сделать скриншот текущей страницы во встроенном браузере. Скриншот будет автоматически прикреплён к следующему сообщению как изображение — модели с vision увидят его и смогут проанализировать визуально. Используй для отладки UI / визуальных регрессий.',
    parameters: { type: 'object', properties: {} }
  },
  {
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        provider_id: {
          type: 'string',
          description: 'Опционально — провайдер для sub-agent (grok / grok-cli). Если не указан — берётся из default_provider скилла, иначе используется провайдер текущего чата.'
        },
        model: {
          type: 'string',
  ...
  },
  {
    ...
    description: 'Запускает несколько подзадач параллельно. Каждая задача — независимый sub-agent С ИНСТРУМЕНТАМИ (agent-loop): он сам читает файлы / ищет по проекту, а с ролью executor/verifier правит код и запускает проверки. Provider задаётся НА КАЖДУЮ задачу — в одном батче можно СМЕШИВАТЬ grok (API) и grok-cli (Grok Build CLI). Поддерживает роли (planner/critic/executor/verifier/researcher) — роль определяет и поведение, и whitelist инструментов. Субагенты (executor/researcher/planner) могут делегировать дальше под лимитом глубины и числа агентов. Используй для параллельного анализа, кросс-верификации, исследования нескольких зон. До 10-12 задач с контролем concurrency.',
    parameters: {
      type: 'object',
      properties: {
        tasks: {
          ...
          items: {
            ...
            properties: {
              ...
              provider_id: { type: 'string', description: 'Провайдер для ЭТОЙ задачи (опц. — по умолчанию текущий). grok (API) и grok-cli (Grok Build) можно смешивать в одном батче.' },
